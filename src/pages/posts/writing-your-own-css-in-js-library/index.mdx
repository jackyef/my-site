...
inside this `CSSStyleSheet`. It is an array-like object, which means we can do some stuff we can do to an array, like accessing the
...
Alright, we have our basic stuff working so far! Now, we want to make it so we can write css with [tagged template literal](https://wesbos.com/tagged-template-literals).
