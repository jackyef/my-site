...
import { ProfileCardToHero } from './_components/ProfileCardToHero';
...
1. In most cases, the animated element isn't actually one element. They are 2 totally different elements,
   though representing a same entity; in this case, a `Task`
2. The animation can not be statically defined. The position of the element is dynamic.
   The amount of pixel we need to move the element by is dependendent on
...
In various parts of the web-app, there are `ProfileCard` components which look like the following:
...
You aren't even sure if it's possible to implement it. You go for a lunch to clear off your mind. You need to take a break
...
second state, it is positioned at `x: 452, y: 255`. You also now that the first profile picture has a size of
`height: 60, width: 60` and the second one has a size of `height: 120, width: 120`.

### Invert

With that information, you can calculate the transformation you need, to make it seem like that the profile picture in the
second state is exactly the same as the first state.

```jsx
// In practice, only one of img1 and img2 will exist at the same time,
// since one of the element will be removed from the DOM already when the other one appears.
// You should cache the DOMRect of the previous state somewhere to make it work.
// In this snippet, we assume img1 and img2 both exist in the same time for the sake of simplicity.
...
Notice that with the inverting transformation, you have made the profile picture in the second state to look exactly
...
be restored to where it is supposed to be placed. This as simple as it sounds, just remove the transformation! Though, we need to
...
you are done with it. Not to mention that you also need to use `requestAnimationFrame()` to schedule them correctly.
...
Surprised by the bug, you apologize and tell the team that you will work on a fix ASAP. You lost your confidence immediately,
...
2. Calculate `deltaCenterpointX` and `deltaCenterpointY` instead of just `deltaX` and `deltaY`. This is needed to handle
   cases where the 2 states have different aspect ratio.
...
you can check them out [here](TODO:addlink).

If you are looking for a more robust react-based library to do this, I'd
suggest taking a look at at [`framer-motion`'s `layoutId`](https://www.framer.com/docs/component/###layoutid) and/or
