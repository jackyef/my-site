...
to `true`. Try listening to the audio below. The code for it is essentially, but with some custom
controls UI for it.
...
One thing that we care about it is that it is able to play audio loops without the annoying gap
that we have in `<audio>` element.

### Playing audio file

Compared to the `<audio>` element, the Web Audio might feel quite a bit more complicated. To get started,
...
[The MDN docs page explains this more in-depth](https://developer.mozilla.org/en-US/docs/Web/API/AudioNode).
We don't need them for our use case, but it is useful to see how powerful the Web Audio API is. You
...
before we are able to play an audio file with the Web Audio API, we need to download the whole audio file beforehand!
...
is still preferred unless there is a specific use case that requires the Web Audio API.
