...
party JavaScripts, either via `npm`, bundled into our code, or via `<script>`
tag (e.g.: Google Analytics, etc). Embedding 3rd party images/media is also
pretty common. Even linking to a 3rd party sites can have security implications!
...
where the opened page gains access to `window.opener`, but this time it is the
3rd party script that gains access to the full `window` object. There are many
...
blocking the stored XSS scripts, we can do that by using a `nonce`.
